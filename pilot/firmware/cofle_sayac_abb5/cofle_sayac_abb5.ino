 ...
 * ============================================================
 *
 *  HEDEF:  ABB5 robotunun 3 dijital çıkışını izler:
 *            - İstasyon 1 sayaç (her parça = 1 pulse)
 *            - İstasyon 2 sayaç (her parça = 1 pulse)
 *            - Robot çalışıyor durumu (HIGH=çalışıyor)
 *
 *  PIN ATAMALARI:
 *   - GPIO25  → Röle 1 NO  (İstasyon 1 — BEYAZ tel)
 *   - GPIO26  → Röle 2 NO  (İstasyon 2 — GRİ tel)
 *   - GPIO27  → Röle 3 NO  (Robot durumu — KAHVE tel)
 *   - GND     → 3 röle COM ortak (SARI tel)
 *
 *  ABB2'den FARKI: sadece CIHAZ_ID + ROBOT_NO
 * ============================================================ */
...
void setup() {
  ...
  Serial.println("[READY] ABB5 3 girisli sayac aktif — robot sinyalleri bekleniyor...\n");
  ledYakBlink(3, 60);
}
