/* ============================================================
 *  COFLE PILOT SAYAC — PLASTIK-YAPISTIRMA firmware (v2.1)
 *  >>> OTOMATIK URETILDI: generate.py — manuel duzenleme!
 * ============================================================
 *
 *  HEDEF:  Yapistirma yapistirma makinesinin uretim rolesini izler.
 *          Her yapistirma cevriminde role KISA sure GND'ye kapanir -> 1 pulse.
 *          (Sinyal KISA pulse — montaj/pres gibi; bolum "plastik", TK1 hatti.)
 *
 *  PIN ATAMALARI:
 *   - GPIO25  -> Role NO  (Uretim sinyali — kuru kontak, GND'ye kapanir)
 *   - GPIO26  -> BOS (kullanilmiyor)
 *   - GPIO27  -> BOS (kullanilmiyor)
 *   - GND     -> Role COM
 *
 *  UYARI: Sinyal KURU KONTAK olmali. 24V ise araya optokuplor (ESP32 3.3V).
 *  AYAR: KISA pulse ayari (INTEG_YUKSEK=15 ~75ms). MIN_PULSE_GAP makinenin cevrim
 *        hizina gore sahada ince ayar isteyebilir (heartbeat TANI ile — asagi bak).
 *
...
// ════════════════════════════════════════════════════════════
//   YAPILANDIRMA — Yapistirma
// ════════════════════════════════════════════════════════════

const char* CIHAZ_ID  = "PLASTIK-YAPISTIRMA";
const char* BOLUM     = "plastik";
const char* ROBOT_NO  = "Yapistirma";
...
// YAPISTIRMA rolesi her cevrimde KISA sure GND'ye kapanir (sayac pulse'u gibi, ~50-200ms).
// Bu yuzden metal/plastik-enjeksiyonun UZUN (~4sn) esigi DEGIL, montaj/pres gibi KISA esik:
...
// (~5ms dongu periyodu: 15 ornek ~= 75ms net LOW — tek glitch pulse'i oldurmez.)
// SAHA AYARI: makine cok kisa (<50ms) pulse veriyorsa YUKSEK_ESIK'i dusur (orn. 8);
// cift/yankili sayiyorsa MIN_PULSE_GAP'i buyut. Heartbeat TANI (tani_sayildi/parazit/erken)
// ile karari sunucudan gozle (metal 400T'de oldugu gibi).
const int  INTEG_YUKSEK_ESIK   = 15;   // ~75ms net LOW -> pulse algilandi (montaj/pres ile ayni)
const int  INTEG_MAX           = 40;   // tavan (~200ms) — glitch toleransi tamponu
const int  INTEG_DUSUK_ESIK    = 4;    // ~20ms HIGH -> pulse bitti (histerezis)
const unsigned long MIN_PULSE_GAP_MS = 1500; // iki gecerli sayim arasi min (cift sayimi eler; sahada ayarla)
...
const char* FIRMWARE_VER  = "2.6.2-yapistirma";   // 2.6.2: WDT+OTA fix, non-blocking reconnect, retry backoff, WiFi-down hard reset, churn onleme
...
void setup() {
  ...
  Serial.println("╚════════════════════════════════════════════╝");
  Serial.printf("FW: %s · Bolum: %s · Makine: %s · YAPISTIRMA (role kisa pulse)\n",
                FIRMWARE_VER, BOLUM, ROBOT_NO);
  Serial.printf("Uretim pini: GPIO%d (role her cevrimde kisa pulse)\n", PIN_IST1_SAYAC);
  ...
  robotCalisiyor = false;  // Yapistirmada robot durumu kullanilmiyor
