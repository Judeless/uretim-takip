/* ============================================================
 *  COFLE PILOT SAYAC — ABKANT-A2 firmware (v2.1)
 *  >>> OTOMATIK URETILDI: generate.py — manuel duzenleme!
 * ============================================================
 *
 *  HEDEF:  Abkant 2 abkant makinesinin ProManage rolesini izler.
 ...
 *   - GPIO27  -> BOS (buzzer kodu duruyor ama abkant'ta buzzer TAKILMAZ)
 ...
 *
 *  Bu dosya pilot/firmware/_templates/abkant.ino.tpl'den uretildi.
 * ============================================================ */
...
// ════════════════════════════════════════════════════════════
//   YAPILANDIRMA — Abkant 2
// ════════════════════════════════════════════════════════════

const char* CIHAZ_ID  = "ABKANT-A2";
const char* BOLUM     = "pres";      // abkant, pres bolumunun makinesidir
const char* ROBOT_NO  = "Abkant 2";
...
const int PIN_IST1_SAYAC      = 25;   // ROLE (kuru kontak, GND'ye kapanir — ProManage)
const int PIN_IST2_SAYAC      = 26;   // BOS
const int PIN_BUZZER          = 27;   // BOS (abkant'ta buzzer TAKILMAZ; kod duruyor, zararsiz)
const int PIN_LED             =  2;
...
const unsigned long MIN_PULSE_GAP_MS = 1500;   // Abkant: iki buk arasi min (sahada ayarlanabilir — cift sayimi eler)
...
const char* FIRMWARE_VER  = "2.7.2-abkant_2";   // 2.7.2: WDT+OTA fix, non-blocking reconnect, retry backoff, WiFi-down hard reset, churn onleme
...
void setup() {
  ...
  Serial.println("╚════════════════════════════════════════════╝");
  Serial.printf("FW: %s · Bolum: %s · Makine: %s · ABKANT ROLE MODU\n",
  ...
  Serial.println("[READY] Integrator debounce + TANI aktif — abkant role sinyali bekleniyor...");
